...
import styles from '../../../../../styles/componentsettings/FaqRowStyleSelectorSetting.module.css';
...
const FaqRowStyleSelectorSetting = () => {
  ...
  useEffect(() => {
    ...
    const _mainClassName = tempStyleCtx?.styleClasses?.childClassName[0] || []; 
  ...
  const handleStyleSlector = (val:string, e:any, selector:string="") => {
    ...
    if(tempStyleCtx?.styleClasses?.childClassName && tempStyleCtx?.styleClasses?.childClassName[0]){
      tempStyleCtx.styleClasses.childClassName[0].push(_obj);
    }else{
      tempStyleCtx.styleClasses.childClassName[0] = [_obj];
    }
  ...
  const removeStyleSel = (aIdx:number) => {
    ...
    let _index = tempStyleCtx?.styleClasses?.childClassName[0]?.findIndex((x:any) => x.label === addStyleSel[aIdx]?.label);
    tempStyleCtx?.styleClasses?.childClassName[0].splice(_index, 1);
    setStyleOfElement(tempStyleCtx);
  ...
  return (
    <Fragment>
      <div className={`${styles.subSettingTitle} ${selectedSetting === "faq_row_style_sel_gen_setting" ? "active_subsetting" : ""}`} onClick={()=>displaySubSetting("faq_row_style_sel_gen_setting")}>
        Style Selector Settings
      </div>
      <div style={{ display:`${selectedSetting === "faq_row_style_sel_gen_setting" ? "block" : "none"}`, backgroundColor:"#0e0e0e" }}>
...
export default FaqRowStyleSelectorSetting;
