...
import styles from '../../../../../styles/componentsettings/Border.module.css';
...
const Border = () => {
  ...
  const handleChange = (color:any) => {
    ...
    const borderWidth = getStylesFromCtx("border-width");
    let bStyle = getStylesFromCtx("border-style");
  ...
  useEffect(() => {
    ...
    let borderWidth = getStylesFromCtx("border-width");
    ...
    const borderColor = getStylesFromCtx("border-color");
    if(borderColor){
    ...
    const bStyle = getStylesFromCtx("border-style");
    ...
    let _radius = getStylesFromCtx("border-radius");
  ...
  const setBorderType = (e:React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if(e.target.value !== 'None'){
      ...
      let bColor = getStylesFromCtx("border-color");
  ...
  const setBorder = (position:string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch(position){
      ...
      case "radius":
        ...
        const radType = changeStyleOfElement?.type === "Image" ? "%" : "px";
        const retRad = setStylesToCtx(["border-radius"], [`${parseInt(e.target.value)}${radType}`]);
        tempStyleCtx = setClassesName(retRad, tempStyleCtx);
  ...
  return (
    <Fragment>
      <div className={`${styles.subSettingTitle} ${selectedSetting === "border_setting" ? "active_subsetting" : ""}`} onClick={()=>displaySubSetting("border_setting")}>
        Border Settings
      </div>
