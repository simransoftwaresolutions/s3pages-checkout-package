...
import styles from '../../../../../styles/componentsettings/BannerImageSetting.module.css';
...
const BannerImageSetting = () => {
  ...
  useEffect(() => {
    ...
    const init = async() => {
      if(bannerCollectionId !== undefined){
        ...
        if(qfieldRef && qfieldRef?.current && bannerCollectionId !== undefined) qfieldRef.current.value = tempStyleCtx?.imageIdx;
      }
    }
  ...
  const handleFieldName = (val:any) => {
    ...
    if(collectionData?.collection_data?.length){
      ...
      tempStyleCtx.bannerData.image = que;

      tempStyleCtx.imageIdx = val;
  ...
  return (
    <Fragment>
      <div className={`${styles.subSettingTitle} ${selectedSetting === "banner_img_setting" ? "active_subsetting" : ""}`} onClick={()=>displaySubSetting("banner_img_setting")}>
        Image Settings
      </div>
      <div style={{ display:`${selectedSetting === "banner_img_setting" ? "block" : "none"}`, backgroundColor:"#0e0e0e" }}>
          ...
          (
            <Fragment>
              <div className={`${styles.mainContainer} inner_setting`}>
                <Select selRef={qfieldRef} format={OutlinedFormat} label="Image Field" items={fieldSelctData} onChange={(e:any)=>handleFieldName(parseInt(e.target.value))} />
              </div>
            </Fragment>
          ):(<></>)
        }
