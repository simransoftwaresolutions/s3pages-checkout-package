...
import styles from '../../../../../../styles/editorui/CommonBackground.module.css';
import { Select, Text } from '../../../../atoms/Input';
import { useState, useEffect, useRef } from 'react';
import { BackgroundTypeItems, BackgroundPosItems, BackgroundSizeItems, BackgroundRepeatItems, BgGradientTypeItems } from '../../../../datas/commonComponentData';
...
const CommonBackground = ({componentNumber, activeComponent, setActiveComponent}:CommonBgProps) => {
  ...
  useEffect(() => {
    ...
    const _backgroundImgClr = getStylesFromCtx("background-blend-mode", componentNumber);
    if(_backgroundImgClr && _backgroundImgClr === "multiply"){
      const _bgColorImageColor = getStylesFromCtx("background-color", componentNumber);
      if(_bgColorImageColor){
        setColorImageColor(_bgColorImageColor);
      }else{
        setColorImageColor("#fff");
      }
      const _bgColorImage = getStylesFromCtx("background-image", componentNumber);
      if(_bgColorImage){
    ...
    const _background = getStylesFromCtx("background", componentNumber);
  ...
  useEffect(()=>{
    ...
    if(colorImage && colorImageColor){
      ...
      const styleNameArr = ["background-blend-mode", "background-color", "background-image", "background"];
      const styleValArr = ["multiply", colorImageColor, _colorImage, ""];
  ...
  const setBackgroundColorStyle = (type:string, color:any="", colorType:string="") => {
    ...
    if(backgroundInfo){
      const ret = setStylesToCtx(["background-blend-mode", "background"], ["normal", backgroundInfo], componentNumber);
      tempStyleCtx = setClassesName(ret, tempStyleCtx, componentNumber);
    } 
  ...
  return (
    <Fragment>
      <div className={`${styles.subSettingTitle} ${open ? "active_common_settings" : ""}`} onClick={()=>setOpen(!open)}>
        Background Settings <span style={{ float:"right" }}>{open ? <KeyboardArrowUpIcon fontSize='medium'/> : <KeyboardArrowDownIcon fontSize='medium'/>}</span>
      </div>
