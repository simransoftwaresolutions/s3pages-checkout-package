...
import styles from '../../../../../../styles/editorui/RowBackground.module.css';
...
const RowBackground = () => {
  ...
  useEffect(() => {
    ...
    const _backgroundImgClr = getStylesFromCtx("background-blend-mode", 0);
    if(_backgroundImgClr && _backgroundImgClr === "multiply"){
      const _bgColorImageColor = getStylesFromCtx("background-color", 0);
      if(_bgColorImageColor){
        setColorImageColor(_bgColorImageColor);
      }else{
        setColorImageColor("#fff");
      }
      const _bgColorImage = getStylesFromCtx("background-image", 0);
      if(_bgColorImage){
    ...
    const _background = getStylesFromCtx("background", 0);
  ...
  const setBackgroundColorStyle = (type:string, color:any="", colorType:string="") => {
    ...
    if(backgroundInfo){
      const ret = setStylesToCtx(["background-blend-mode", "background"], ["normal", backgroundInfo], 0);
      tempStyleCtx = setClassesName(ret, tempStyleCtx, 0);
    } 
  ...
  return (
    <Fragment>
      <div className={`${styles.subSettingTitle} ${selectedSetting === "row_bg_setting" ? "active_subsetting" : ""}`} onClick={()=>displaySubSetting("row_bg_setting")}>
        Background Settings
      </div>
      <div style={{ display:`${selectedSetting === "row_bg_setting" ? "block" : "none"}`, backgroundColor:"#0e0e0e" }}>
